...
export const AddAttachments = ({ ...props }: ButtonProps) => {
	const attachments = usePromptInputAttachments();
...
export const PromptInput = ({
	...
	const convertBlobUrlToDataUrl = async (url: string): Promise<string> => {
		...
		// Resize images (ignoring SVGs) to a max of 400px
		if (blob.type.startsWith("image/") && !blob.type.includes("svg")) {
			return new Promise((resolve, reject) => {
				...
				img.onload = () => {
					...
					resolve(canvas.toDataURL(blob.type, 0.85)); // 0.85 quality for a good balance of size and visually lossless
				};
				img.onerror = () => {
...
export const PromptInputSubmit = ({
	...
	return (
		<InputGroupButton
			aria-label="Submit"
			className={cn("bg-foreground text-background hover:bg-foreground/90 shadow-sm rounded-full", className)}
			size={size}
			type="submit"
			variant={variant}
			{...props}
